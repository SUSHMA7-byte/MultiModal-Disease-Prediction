...
model_path_heart = r"D:\FINAL_YEAR_PROJECT\PickleFiles\heart.pkl"
heart = joblib.load(model_path_heart)
model_path_diabetes =r"D:\FINAL_YEAR_PROJECT\PickleFiles\diabetes.pkl"
diabetes = joblib.load(model_path_diabetes)
model_path_liver=r"D:\FINAL_YEAR_PROJECT\PickleFiles\liver_yes_no.pkl"
liver=joblib.load(model_path_liver)
model_path_kidney=r"D:\FINAL_YEAR_PROJECT\PickleFiles\Chronic_Kidney.pkl"
kidney=joblib.load(model_path_kidney)
model_path_Thyroid=r"D:\FINAL_YEAR_PROJECT\PickleFiles\Thyroid.pkl"
thyroid=joblib.load(model_path_Thyroid)
model_path_stroke=r"D:\FINAL_YEAR_PROJECT\PickleFiles\stroke.pkl"
stroke=joblib.load(model_path_stroke)
model_path_copd=r"D:\FINAL_YEAR_PROJECT\PickleFiles\copd.pkl"
...
#brain_stroke
@app.route('/brain_diseases')
def brain_diseases():
    return render_template('brain_diseases.html')

@app.route('/predict/<disease>')
def predict(disease):
    # Your prediction logic goes here
    return f"Predicting for {disease}"
...
@app.route('/make_prediction3', methods=['POST'])
def make_prediction_kidney():
    if request.method == 'POST':
        features = [float(x) for x in request.form.values()]
        input_data = np.array(features).reshape(1, -1)
        prediction = kidney.predict(input_data)
        result = "Has Chronic Kidney Disease" if prediction[0] == 1 else "Healthy"
        return render_template('prediction_kidney.html', result=result)
...
# Drug Recommendation
sym_des = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\symtoms_df.csv")
precautions = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\precautions_df.csv")
workout = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\workout_df.csv")
description = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\description.csv")
medications = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\medications.csv")
diets = pd.read_csv("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\dataset\diets.csv")

svc = pickle.load(open("D:\FINAL_YEAR_PROJECT\Life care\Medicine-Recommendation-System\models\svc.pkl",'rb'))
